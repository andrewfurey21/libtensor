...
TEST(Tensor, Add3x4) {
  ...
  ASSERT_TRUE(intarray_equal(output->dview->shape, tensor1->dview->shape));
  ASSERT_TRUE(intarray_equal(output->dview->shape, tensor2->dview->shape));
...
TEST(Tensor, Sum3x4) {
  ...
  ASSERT_TRUE(intarray_equal(output->dview->shape, output_shape));
...
TEST(Tensor, Relu3x4) {
  ...
  ASSERT_TRUE(intarray_equal(output->dview->shape, shape1));
  bool equal = tensor_equal(correct, output, 1e-5, 1e-8);
...
TEST(Tensor, Reshape3x4) {
  ...
  ASSERT_TRUE(intarray_equal(output->dview->shape, new_shape));
  bool equal = tensor_equal(correct, output, 1e-5, 1e-8);
